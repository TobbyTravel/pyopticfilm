...
- **GL128 consecutive-scan AFE strip timeout**: after an image pass, stationary AFE START could hang for 5s at status `0xcd` (home, buffer empty, lamp on, motor still enabled). Scan Lab reported `AFE strip: no stationary data ready within 5s (last status=0xcd)`. Stationary acquires now clear SCAN and motor, wait until idle at home, write SCAN absolutely (same as shading strips), and retry START once.
