...
**⚠ RETRACTED 2026-09-06 — LINCNT "semantics bug" does not exist.** An earlier version of
this document (and the Aug-30 `hw-ref/8100v2-capture-analysis` branch's finding C-013)
claimed `image_lincnt_per_line`/`usb_image_lincnt_half_lines` were wrong for V2, based on
arithmetic that divided the vendor driver's raw bulk bytes by the register LINCNT value
and got exactly 1 byte/sample. That arithmetic is correct but doesn't prove what it was
used to prove: it only confirms the driver used 8-bit depth, not that the register value
equals final image height rather than 4× it. **Actually running pyopticfilm's own,
unmodified `compute_geometry(7200, model=MODEL_8100_V2)` reproduces the captured register
value (29012) and byte total (902,389,248) exactly**, using the inherited ×4 convention
untouched — see the LINCNT row below for the full account. No code change needed here.
Left in this document (rather than deleted) as a record of the correction, since the
initial "confirmed, high priority" framing was itself wrongly stated with high confidence
and is worth learning from: **treat "N independent data points agree" as validating the
raw numbers, not necessarily the interpretation put on them** — always cross-check an
architectural conclusion against what the current code actually computes before trusting
arithmetic alone.
...
| LINCNT (0x25-0x27) semantics / `image_lincnt_per_line` | inherits SE ×4/half-lines convention | V2 uses output-line-count directly (C-013) | ×4, half-lines (SE) | **RETRACTED, see below.** Register value 29012 is real and reproduced 13 times across sessions 04/06/07 (not in question) — what was wrong was the *interpretation* of what it means. | **⚠ 2026-09-06 correction: this "bug" does not exist. Current code is already correct.** Running `compute_geometry(7200, model=MODEL_8100_V2)` **unmodified, on today's `main`**, gives `lines=7253` (asic-native height from 25.59mm ÷ 25.4 × 7200dpi), `lincnt_per_line=4` (inherited, untouched), **`register_lincnt=29012`** — exactly matching the captured register value — and `total_bytes=902,389,248`, exactly matching the captured vendor bulk total. The earlier "no ×4 factor" conclusion (both this session's extraction pass and Aug-30's C-013) came from dividing the vendor driver's raw delivered bulk bytes by the register value and getting exactly 1 byte/sample — true, but that only confirms the driver used 8-bit depth (already known from the DEPTH row below), it does **not** independently measure final image height, so it can't actually distinguish "register = final height" from "register = 4× final height, decimated later." Actually running the code resolves it: the ×4 convention holds for V2 exactly as inherited, and the exact match to the captured value is not attainable by coincidence. **No code change needed. Retracted from Phase 4.3 — nothing to fix here.** |
