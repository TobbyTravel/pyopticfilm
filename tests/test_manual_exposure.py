...
def test_me_target_exposure_clamped_to_v2_n_bracket_ceiling_off_7200dpi():
    """V2 N-bracket (n_brackets > 2) pins the long ceiling at 42000 every DPI."""
...
def test_me_target_exposure_v2_two_bracket_keeps_shared_ceiling():
    """n_brackets == 2 on V2 still uses the shared 85k non-7200 envelope."""
    session, _usb = _mock_gl128_session_armed_for(MODEL_8100_V2)
    session.run(
        resolution=1800,
        area=_TINY,
    ...
    assert session.last_me_debug.exposure_long == 80000
