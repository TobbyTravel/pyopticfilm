...
REGISTERS: tuple[RegisterEntry, ...] = (
    ...
    RegisterEntry(
        addr="0x1D",
        name="Unnamed, constant in SCAN_REGS (0x80) except at 7200dpi",
        asic=AsicFamily.GL128,
        scope=(SCOPE_ALL_GL128,),
        meaning=(
            "Not in Gl128Registers or any per-DPI table; pyopticfilm always "
            "writes 0x80 (SCAN_REGS) regardless of DPI. Capture register-diff "
            "across every PPI-ladder DPI shows the real vendor driver's byte "
            "flip to 0x81 (bit 0 set) only for the 7200dpi image pass, on both "
            "the 8100 V2 (06_ppi_ladder + 04_color_7200) and the 8200i SE "
            "(13_ppi_ladder) independently — never at any other DPI, never "
            "during shading. Meaning still unknown. A real-hardware test that "
            ...
            "5-bit LINESEL field at bits 0-4 (0x1f). Our observed 0x80→0x81 "
            "at 7200dpi is precisely CK4LOW-held | LINESEL 0→1. As a sanity "
            ...
            "highest DPI (7200, oversample=1) — the opposite end of the "
            "range. Plustek's GL128 firmware and this film scanner's motor/"
        ...
        citations=_c(
            ...
            "Independent capture register-diff across pyopticfilm_captures "
            "8100-v2/06_ppi_ladder + 04_color_7200 and 8200i-se/13_ppi_ladder",
    ...
    RegisterEntry(
        ...
        meaning=(
            ...
            "unlike 0x1D, none of these are even known to vary with DPI or "
            "mode (only checked across the PPI ladder; not checked against "
            "ME, IR, or crop variation). This entry exists so the gap is "
            "visible in one place rather than silently absent from the "
            "catalog, not because anything here is suspected of doing "
            ...
            "constant value across INIT_REGS and SCAN_REGS.\n\n"
